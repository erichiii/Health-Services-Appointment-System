...
/**
 * Get available dates for a specific service category/subcategory
 */
function getAvailableDatesForService($category, $subcategory = null, $months_ahead = 3)
{
    global $pdo;
    ...
    try {
        ...
        if ($subcategory && isset($serviceMap[$subcategory])) {
            // Look for specific service
            $query_conditions[] = "s.name = ?";
            $query_params[] = $serviceMap[$subcategory];
        } else {
            // Look for any service in the category
            $query_conditions[] = "s.category = ?";
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ss.schedule_date,
                s.name as service_name,
                GROUP_CONCAT(CONCAT(TIME_FORMAT(ss.start_time, '%h:%i %p'), ' - ', TIME_FORMAT(ss.end_time, '%h:%i %p')) SEPARATOR ', ') as time_slots,
                SUM(ss.max_appointments - COALESCE(booked.count, 0)) as available_slots
            FROM service_schedules ss
            JOIN services s ON ss.service_id = s.id
            LEFT JOIN (
                SELECT service_schedule_id, COUNT(*) as count 
            ...
            AND s.is_active = 1
            AND {$where_clause}
            GROUP BY ss.schedule_date, s.name
        ...
        while ($row = $stmt->fetch()) {
            ...
            $available_dates[] = [
                'value' => $date_key,
                'display' => $formatted_date . ' (' . $row['available_slots'] . ' slots available)',
        ...
        error_log("Database error in getAvailableDatesForService: " . $e->getMessage());
        return [];
    }
}
